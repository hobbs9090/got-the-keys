...
RSpec.describe "Header account details", type: :request do
  ...
  it "groups the guest register and sign-in buttons beside the language selector" do
    ...
    expect(parsed_html.at_css('link[rel="preload"][as="style"][href*="/assets/public-"]')).to be_present
    expect(parsed_html.at_css('link[rel="stylesheet"][href*="/assets/public-"][media="print"][onload="this.media=\'all\'"]')).to be_present
  ...
  it "exposes a root home-link without aria-current so redundant root visits can be skipped in JS" do
    ...
    home_link = parsed_html.at_css('[data-testid="home-link"]')
    expect(home_link).to be_present
    expect(home_link["aria-current"]).to be_nil
    expect(home_link["href"]).to eq(root_path)
  end
